...
//Module 6: Bank App. Refactored to include error handling for Module 7 Error Handling Activity
type account struct {
...
func (a *account) withdraw(ammount float64) error {
	if ammount < 0 { //ensure we do not withdraw a negative ammount of money
		return errors.New("You cannot withdraw a negative ammount of money")
	}
	if a.balance >= ammount {
		...
		return errors.New("This account does not have enough money")
	}
	return nil
}

//deposit - deposit money into account
func (a *account) deposit(ammount float64) error {
	if ammount < 0 { //ensure we do not deposit a negative ammount of money
		return errors.New("You cannot deposit a negative ammount of money")
	}
	a.balance += ammount
...
func (a *account) applyInterest() error {
	if a.owner.entityType == "individual" {
		...
		case "savings":
			a.balance *= 1.05
		default:
			return errors.New("Account has an invalid type")
		}
	} else {
		switch a.accType {
		case "checking":
...
func (a *account) wire(target *account, ammount float64) error { //source account is a, the account calling this method
	if a.balance < ammount { //ensure the account has enough money to make the transfer
		return errors.New("The source account's balance is too low")
	}
	if ammount < 0 { //ensure we do not transfer a negative ammount of money
		return errors.New("Cannot wire a negative ammount of money")
...
func (w *wallet) wire(source *account, target *account, ammount float64) error {
	...
	if notFound {
		return errors.New("Source account is not in this wallet")
	}
	...
	if source.balance < ammount {
		for ii := 0; ii < len(w.accounts); ii++ {
			if w.accounts[ii].balance >= ammount {
				...
				errMessage := "The source account does not have enough money to make this transfer. However, account " + strconv.Itoa(w.accounts[ii].accNum) + " does have enough to make the transfer"
				return errors.New(errMessage)
			}
		}
		return errors.New("The source account does not have enough money to make this transfer")
	}
			...
			if input >= len(allAccs) {
				...
				for true {
					...
					case 1:
						...
						fmt.Scanln(&ammount)
						err := allAccs[xx].deposit(ammount)
						if err != nil {
							fmt.Println(err)
							continue
						}
					case 2:
						fmt.Println("How much money?")
						fmt.Scanln(&ammount)
						err := allAccs[xx].withdraw(ammount)
						if err != nil {
							fmt.Println(err)
							continue
						}
					case 3:
						...
						if input2 >= len(allAccs) {
							...
							fmt.Println("How much money?")
							fmt.Scanln(&ammount)
							err := allAccs[xx].wire(allAccs[input2], ammount)
							if err != nil {
								fmt.Println(err)
								continue
							}
				...
				for true {
					...
					case 3:
						...
						if input2 >= len(allAccs) || input3 >= len(allAccs) {
							...
							fmt.Println("How much money?")
							fmt.Scanln(&ammount)
							err := allWall[xx].wire(allAccs[input2], allAccs[input3], ammount)
							if err != nil {
								fmt.Println(err)
								continue
							}
